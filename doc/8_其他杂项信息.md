...
!function () {
    let seat = 0, index = 0;
    uiscript.UI_DesktopInfo.Inst._player_infos[seat].emo.show(view.DesktopMgr.Inst.seat2LocalPosition(seat), index);
}();
```

两个变量: `seat` 和 `index`, `seat` 自然是玩家的座次,
...
```js
!function () {
    let effect_paths = ['scene/effect_yiman_jihui_chs.lh', 'scene/effect_yiman_queding_chs.lh'];
    let sounds = [239, 240];
    for (let i = 0; i < 2; i++)
        Laya.timer.once(1800 * (i + 1), this, function() {
            ...
            view.AudioMgr.PlayAudio(sounds[i]);
        });
}();
```
...
!function(){
    ...
    for (let i in names) 
        Laya.timer.once(1600 * (parseInt(i) + 1), this, function() {
            uiscript.UI_DesktopInfo.Inst['open' + names[i] + 'BeginEffect']();
        });
}();
```
