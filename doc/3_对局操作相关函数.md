...
## 切牌: `qiepai(seat, tile, is_liqi, f_moqie)`
...
- 若是通过 `qiepai_set` 获取的牌, 则为摸切
- 若不是通过 `qiepai_set`, 则手动给参数 `tile` 赋值表示手切, 不赋值表示摸切

实际切的牌按照以下优先级顺序确定

1. 给 `tile` 赋值的牌
2. `qiepai_set` 中 `seat` 号玩家的切牌顺序
3. 摸切(切最右侧的牌)
...
## 鸣牌(暗杠/加杠/拔北): `zimingpai(seat, tile, type)`
...
与 `qiepai_set` 配合使用可以代替无人鸣牌情况下的冗长而杂乱的切牌
...
其次 `qiepai()` 通常也有所指定, 故一般也与 `qiepai_set` 配合使用
...
函数内容只有 `normalMoqie(cnt)` 和 `huangpai()`, 其中 `cnt` 是固定值, 即当前位置牌山的余牌数量
