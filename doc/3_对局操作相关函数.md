...
`mopai(seat, tile)`, 参数顺序可以不一致

`seat` : 摸牌的玩家, 没有此参数时由脚本判断哪位玩家摸牌, 是否为岭上摸牌也由脚本自动判断
...
`qiepai(seat, tile, is_liqi)`, 参数顺序可以不一致

`seat` : 切牌的玩家, 没有此参数时由脚本判断哪位玩家切牌

`tile` : 要切的牌, 若没有此参数, 则为摸切
...
`is_liqi` : 表示这次切牌是否立直(`true` 或 `false`), 由脚本自动判断是否为双立直, 若参数为 `'kailiqi'`, 则为开立直
...
`mingpai(seat, tiles)`, 参数顺序可以不一致

吃/碰/明杠 都是**其他家摸牌并切牌**后的鸣牌操作

`seat` : 吃/碰/明杠 的玩家, 没有此参数时由脚本按照逆时针顺序判断是哪位玩家
...
`zimingpai(seat, tile, type)`, 参数顺序可以不一致

暗杠/加杠/拔北 都是**自家摸牌**后的鸣牌操作

`seat` : 暗杠/加杠/拔北 的玩家, 没有此参数时由脚本判断哪位玩家暗杠/加杠/拔北
...
若没有该参数, 则除了"三家和了"外, 由脚本自动判断属于哪种流局(即"三家和了"目前只能通过手动输入参数的方式实现)
...
详见该文件夹下的 [天凤牌谱编辑器数据转雀魂格式](../products/根据可见手牌和牌河生成雀魂牌谱/天凤牌谱编辑器数据转雀魂格式.md) 说明
