...
- 若是通过 `qiepaiset` 获取的牌, 则为摸切
- 若不是通过 `qiepaiset`, 则手动给参数 `tile` 赋值表示手切, 不赋值表示摸切

实际切的牌按照以下优先级顺序确定

1. 给 `tile` 赋值的牌
2. `qiepaiset` 中 `seat` 号玩家的切牌顺序
3. 摸切(切最右侧的牌)
...
与 `qiepaiset` 配合使用可以代替无人鸣牌情况下的冗长而杂乱的切牌
...
其次 `qiepai()` 通常也有所指定, 故一般也与 `qiepaiset` 配合使用
