...
!function () {
    const names = [
        ...
        'yongchang_begin',
        'hunzhiyiji_begin',
        'wanxiangxiuluo_begin',
        'beishuizhizhan_begin',
    ];
    for (let i in names) {
        Laya.timer.once(1600 * (parseInt(i) + 1), this, function () {
            let front_effect = game.FrontEffect.Inst.create_ui_effect(uiscript.UI_DesktopInfo.Inst.me.getChildByName('container_effects').getChildByName('dora3_begin'), 'scene/effect_'+ names[i] + '_chs.lh', new Laya.Point(0,0), 1);
            view.AudioMgr.PlayAudio(243);
            Laya.timer.once(5000, front_effect, function() {
                front_effect.destory();
            });
        });
    }
}();
