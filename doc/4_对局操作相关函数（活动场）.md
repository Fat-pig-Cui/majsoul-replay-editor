...
川麻中需要手动调用该函数的情况只有一个: 开杠收取玩家点数时击飞完场, 在 `leimingpai` 之后调用该函数可以实现该情况
...
State "广义切牌组" as S040 #LightSkyBlue {
    ...
    State "开牌并成功\nunveil()" as S13 #LightSalmon
    State "开牌后锁定\nunveil_lock()" as S14 #DarkSeaGreen
...
S4 -[#DarkCyan,bold]-> S12 : 川麻刮风\n刮风击飞
S5 -[#DarkCyan,bold]-> S12 : 川麻刮风\n下雨击飞
