...
函数内容只有 `mingpai()` 和 `qiepai()` 的循环, `tls_cnt` 比 `normalmoqie(tile_cnt)` 中的 `tile_cnt` 含义更广
...
用于连续的岭上摸牌, 函数内容只有 `leimingpai()` 和 `mopai()` 的循环, `tile_cnt` 同 `normalmoqie` 中的
...
## 便捷函数: 查看当前位置牌山剩余牌的数量: `get_left_tile_cnt()`

备用函数, 另外还有几个开局显示的剩余牌数
...
函数内容只有 `normalmoqie(cnt)` 和 `notileliuju()`, 其中 `cnt` 就是上面的 `get_left_tile_cnt()`
