...
// tiles: 玩家的起手, paishan: 牌山, discardtiles: 玩家的切牌, scores: 玩家点数
var tiles0 = '', tiles1 = '', tiles2 = '', tiles3 = '';
var paishan = '', discardtiles = ['', '', '', ''], scores = [];
// playertiles: 玩家当时的手牌
var playertiles = [[], [], [], []];
// muyuseats: 拥有目玉的玩家队列
var muyuseats = '';
...
// 随机牌山函数
function randompaishan(paishanhead = '', paishanback = '') {
    ...
    for (let i = 0; i < playercnt; i++) {
        ...
        if (i === ju) {
            if (tiles_len !== 14)
                console.warn('chang: ' + chang + ', ju: ' + ju + ', ben: ' + ben + ', tiles' + i + ' 作为庄家长度不对: ' + tiles_len);
        } else if (tiles_len !== 13)
            console.warn('chang: ' + chang + ', ju: ' + ju + ', ben: ' + ben + ', tiles' + i + ' 作为子家长度不对: ' + tiles_len);
    }
        ...
        while (discardtiles[0].length > 0)
            for (let i = ju + 1; i < ju + 1 + playercnt; i++) {
                ...
                if (discardtiles[i % playercnt].length > 0)
                    paishanhead += pop_discardtiles(i % playercnt);
            }
...
function mingpai(seat, tiles) {
    ...
    function trying(x, seat) {
        let x0 = allequaltile(x[0]).reverse(), x1 = allequaltile(x[1]).reverse(), x2 = [];
        if (x.length === 3) // 明杠
            x2 = allequaltile(x[2]).reverse();
    ...
    if (tiles === undefined) {
        ...
        console.error('chang: ' + chang + ', ju: ' + ju + ', ben: ' + ben + ', xun: ' + xun[seat].length + ': 没有玩家能 mingpai()');
        return false;
    }
...
function hupai(allseats, type) {
    ...
    if (!is_xuezhandaodi() && !is_wanxiangxiuluo() && !is_chuanma() && !is_xueliu()) {
        if (!is_guobiao()) {
            ...
            let ret = [];
            ret.push(hupai_guobiao(allseats[0]));
                ...
                if (paihe[0].tiles.length === 1 && paihe[1].tiles.length === 1 && paihe[2].tiles.length === 1 && paihe[3].tiles.length === 1)
                    if (paihe[0].tiles[0] === paihe[1].tiles[0] && paihe[1].tiles[0] === paihe[2].tiles[0] && paihe[2].tiles[0] === paihe[3].tiles[0])
                        if (tiletoint(paihe[0].tiles[0]) >= 28 && tiletoint(paihe[0].tiles[0]) <= 31) {
                            ...
                            if (liqi != null)
                                ret.data.liqi = JSON.parse(JSON.stringify(liqi));
                        }
    }
...
// 其中 showRecord, showInfo_record, setFanFu 在 guobiao.js 中
...
function firstn_tile(n) {
    ...
    console.error('Illegal input at firstn_tile()');
    return '';
}
...
function lastn_tile(n) {
    ...
    console.error('Illegal input at lastn_tile()');
    return '';
}
...
function pop_discardtiles(seat) {
    ...
    console.error('Illegal input at pop_discardtiles()');
    return '..';
}

// ===========================================

// 返回和 tile 等效的所有牌, 优先把红宝牌和含有 sptile_suf 放到前面
...
function tiletoint(tile, type = false, special_tile = false) {
    ...
    console.error('Illegal input at tiletoint()');
    return -1;
}
...
function inttotile(x, type = false) {
    ...
    console.error('Illegal input at inttotile()');
    return 'Xz';
}
