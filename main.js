var MRE = (function (exports) {
            ...
            if (is_angang) {
                ...
                if (is_chuanma())
                    for (let i = 0; i < player_cnt; i++) {
                        if (i === seat || huled[i])
                            continue;
                        chuanma_gangs.not_over.push({ from: i, to: seat, val: 2000 });
                    }
            }
            else {
                if (is_begin_open())
                ...
                if (is_chuanma() && index === player_tiles[seat].length)
                    for (let i = 0; i < player_cnt; i++) {
                        if (i === seat || huled[i])
                            continue;
                        chuanma_gangs.not_over.push({ from: i, to: seat, val: 1000 });
                    }
            }
            player_tiles[seat].sort(cmp);
    ...
    let cuohu;
    // 何切模式: 主视角要保护的牌(防止切出去)
    let protected_tiles = { seat: 0, tiles: [] };
    /**
     * 各种振听, 有效长度为玩家数, 不超过4
     *
     * 影响振听的因素
